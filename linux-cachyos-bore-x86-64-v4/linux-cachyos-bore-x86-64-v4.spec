%global commit0 5ca2493c51b1cadec102c9c549345b43f669960b
%global shortcommit0 %(c=%{commit0}; echo ${c:0:7})

Name:           linux-cachyos-bore-x86-64-v4
Epoch:          1
Version:        7.1.3
Release:        202607210234%{?dist}
...
%description
Linux kernel with CachyOS patches and BORE scheduler, optimized for x86-64-v4.
...
echo "-cachyos-x86-64-v4" > localversion.10-cachyos

# Architecture optimizations for x86-64-v4
scripts/config -e GENERIC_CPU -d MZEN4 -d X86_NATIVE_CPU --set-val X86_64_VERSION 4

# Apply config options for BORE (non-RT, full preempt)
...
%post
KVER=$(ls /usr/lib/modules/ | grep "cachyos-x86-64-v4" | head -n 1)
/usr/sbin/depmod -a ${KVER}
if [ -x /usr/bin/dracut ]; then
    /usr/bin/dracut --force /boot/initramfs-${KVER}.img ${KVER}
fi
if [ -x /usr/sbin/grubby ]; then
    /usr/sbin/grubby --add-kernel=/boot/vmlinuz-${KVER} --title="CachyOS BORE x86-64-v4" --initrd=/boot/initramfs-${KVER}.img --copy-default
fi
